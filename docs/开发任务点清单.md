...
【当前进度】
- 已完成并推送：T1 后端工程 / T2 数据库与字典种子 / T3 前端工程 / T4 登录认证（含首登强制改密）/ T5 字典管理 / T6 公开报名页 / T7 审核管理台（`0ca572e` server API / `d5b8eee` web 页面 / `b4502e7` docs / `5a065c3` 交接同步）
- 已完成但**尚未提交**：T8 审核状态查询页（PRD F-005）
- 下一个任务点：T9 短信通知提效工具（PRD F-004）
...
**⏳ 未完成**：T9 短信提效 · T10 成员档案 · T11 个人中心 · T12 公告 · T13 Excel 导入 · T14 数据导出 · T15 看板 · T16 反馈入口 · T17 全链路联调上线

**📌 未提交项**：**T8 的全部改动**（后端 3 改 + 2 新增、`web/src/api/recruit.js`、`QueryView.vue`、本清单）还在工作树里 —— 新对话应先按规范提交（建议拆：`feat(server): add public review-status query API…` / `feat(web): implement review-status query page` / `docs: record T8 …`）。⚠️ 另注意工作树里有 4 个**故意不入库**的种子材料与旧承接文档，**切勿 `git add -A`**。
