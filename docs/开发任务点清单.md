...
| POST | `/auth/init-admin` | 公开（仅未初始化时可用） | 建首个超管：`role=2` / `department=0` / `duty=3` / `status=0` / `activated_at=NULL` |
...
- **首登强制改密**：登录时把 `needChangePassword` 写进 Sa-Token 会话；拦截器对未改密账号只放行 `/user/current`、`/user/change-password`、`/auth/logout`（**后端兜底，不只靠前端跳转**）
...
| 首次登录 | `code=200`，`needChangePassword=true` |
...
- 超管由引导页创建后**首次登录仍需改一次密码**（`activated_at=NULL`）——这是任务点自测要点要求的流程，也让运营有"改掉初始密码"的习惯
...
| D35 | 首登强制改密 | `activated_at` 为空即为未激活；登录把标记写进 Sa-Token 会话；**拦截器兜底**只放行 `/user/current`、`/user/change-password`、`/auth/logout`；改密成功写 `activated_at` 并强制登出 | T4 / T11 |
| D36 | 系统初始化引导 | 未初始化（user 表为空）时**所有路由**（含公开页）一律引导到 `/init`；已初始化后 `/init` 不可再进；`/auth/init-admin` 在业务层同样校验"表为空" | T4 |
| D37 | Sa-Token 拦截器白名单 | `/health`、`/error`、`/auth/*`、dev 接口文档路径（`/doc.html`、`/webjars/**`、`/v3/api-docs/**`、`/swagger-ui/**`、`/favicon.ico`）；prod 下接口文档整体关闭，故白名单不构成暴露 | T4 / T17 |
