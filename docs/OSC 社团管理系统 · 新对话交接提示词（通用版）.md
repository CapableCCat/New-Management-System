...
**阶段：纳新主链完成（T1~T11）→ 支撑链完成（T12~T15）→ 信息架构调整（T18、T19 已完成）→ 收尾（T16、T17）**

| 已完成并推送 | T1 后端工程 / T2 数据库与字典种子 / T3 前端工程 / T4 登录认证（含首登强制改密）/ T5 字典管理 / T6 公开报名页 / T7 审核管理台 / T8 状态查询页 / T9 短信提效工具 / T10 成员档案管理 / T11 个人中心（首次启用 MinIO）/ T12 公告系统（富文本双重 XSS 清洗）/ T13 Excel 批量导入 / T14 数据导出 / **T15 基础看板** / **T18 公开端报名链路梳理**（含报名页简介精简为一句话；`90ddafe..0ce15b7`） |
| ------------ | ------------------------------------------------------------ |
| **已完成、未提交** | **T19 内部导航与首页工作台**（**范围变更：统一外壳一起做掉** —— 三套 layout 合并为 `PublicLayout` + `AppShell`；菜单与守卫靠路由 `meta.menu.capability` 单一出处；`roles.js` 按权限矩阵补具名能力、store 上 4 个旧计算属性删除；首页改工作台、撤公告摘要；新增公告铃铛红点 + 一键已读）。T19 页面 42 + 「只改一处」端到端 4 + 回归 98 = 144 项全过 |
| **下一步（按序）** | ① **先提交 T19**（等社长说「提交 T19」，按 web / docs 拆 commit）→ ② **T16 轻量反馈入口** → ③ **T17 联调上线** |
...
**本轮施工依据**：`docs/OSC 社团管理系统 · 开发任务点清单.md` 的「T16 轻量反馈入口」条目（含完整交付物清单）。
**需求依据**：`docs/OSC 社团管理系统 · 产品需求文档（PRD）V1.0.md` 已订正为 **v3.2**（§8.2 页面地图改「场景 + 权限」两层模型、F-001/F-005 的入口与出海口径）。

**T18 / T19（已完成、待提交，要点不在此重复）**：交付物与实测记录见《清单》§4 各自条目，实现层决策见 §6 D109~D122。
⚠️ **T19 的范围比原计划大**：社长当场推翻了「统一外壳不做」，**三套 layout 已合并为 `PublicLayout` + `AppShell`**（D118），
「管理端 / 成员端」两套界面与那两个切换按钮都已不存在 —— 内部导航一律由路由表生成、按权限显隐。

**T16 要点（轻量反馈入口，下一步）**
1. **免登录也能提交**（PRD F-014：报名成功页 + 内部页面都要有入口）→ 需要防刷：复用 T8 的**一次性图形验证码**，并把「反馈提交」放进白名单
2. 落脚点：**工作台那张「意见反馈」占位卡**（T19 已留好位置，标注 T16 待接入）+ 报名结果态给一个入口
3. 管理侧：反馈列表（仅社长团 / 超管，见权限矩阵「反馈查看」），支持标记已处理（`feedback.handled`，T2 已建表 → **零 DDL**）
4. 能力函数已备好：`canSubmitFeedback` / `canViewFeedback`（`constants/roles.js`）—— 路由一加进表，菜单与守卫自动生效
5. **T16 会同时动工作台与报名结果态**，这正是它被排在 T18/T19 之后的原因（避免返工两次）
...
3. **本轮没有任何后端改动**（T19 全在前端：路由 / 外壳 / 能力函数 / 工作台 / 红点）→ **不用重新打包、不用动 Maven**；前端 `npm run dev` 起 5173 即可

**⚠️ 工作树约定（T19 之后）**：T19 的改动（web 12 项 + 新增 `layouts/AppShell.vue`、`router/menu.js`、`components/AnnouncementBell.vue`、`utils/announcementRead.js`，删除 `layouts/MemberLayout.vue`、`layouts/AdminLayout.vue`，加 docs 2 份）**全部还在工作树里，未 commit** ——
提交时按功能拆多个 commit（建议：web 统一外壳与路由声明 / web 工作台与公告红点 / docs 台账），**只 add 明确路径**，`git commit` 与 `git push` 分两条命令。
