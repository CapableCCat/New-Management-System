...
> **进度**：T18、T19 已提交推送（T19 = `dd5b909` web / `80ebc86` docs）；**T16 轻量反馈入口已开发完成并自测通过**
> （接口 27 + 页面 26 + 回归 106 = 159 项，见本条实现结果），待社长说「提交 T16」；下一步是 **T17 全链路联调与上线准备**。
...
| 项 | 实现 |
|---|---|
| 社团简介 | `sys_config.club_intro`，前端按空行分段渲染。**2026-09-26（随 T18）精简**：由社长亲笔 397 字长文改为**一句话**「天津中德开源鸿蒙社·搞开源、作项目、共成长」—— PRD F-001 第 1 步本来就只要求「一句话介绍」，长文属超规格实现（见 §6 D117） |
...
| 结果态（原「成功页」） | 图标 + 提交手机号 + 后端下发文案 + 审核时效 + **两个出口**（去查询 / 已有账号？去登录）。**T18 订正**：它只是报名页的一个状态而非独立页，按钮由后端 `nextAction` 决定（见 §6 D109 / D111） |
...
| 重复提交（待审） | **T18 起改**：`200` + `data.state=ALREADY_PENDING` / `nextAction=QUERY`（原为 `40000` 纯文案，无按钮）；库中姓名未被覆盖 |
| **被拒后重提** | `200 已重新提交`，`state=RESUBMITTED`（**T18 起**：原布尔字段 `resubmitted` 并入 `state`，见 §6 D113）；库中字段全部更新、`status` 回 0、`reject_reason/reviewer_id/reviewed_at` 清空、`created_at` 刷新 |
| 重提时清空选填项 | `major_text/tag_text/province/city` **正确置空**（首轮发现的脏数据 bug 已修，见「注意」） |
| 已通过后再提交 | **T18 起改**：`200` + `data.state=ALREADY_MEMBER` / `nextAction=LOGIN`（原为 `40000 该手机号已通过审核，请直接登录系统`） |
...
- ⚠️ **T18 订正了本任务「手机号重复提交」的口径**：原来把「已提交过报名 / 已通过」当 `40000` 报错处理（只回一句纯文案、没有可点的按钮），
  T18 改为 `200` + `data.nextAction`（`QUERY` / `LOGIN`），前端就地渲染按钮（见 §6 D111 / D113）。上表已就地标注订正处，其余原始自测数据不变
...
> ⚠️ **T19 订正**：首页那张「公告摘要卡」**已撤掉**（D107 —— 首页改工作台，公告走「公告」菜单 + 右上角铃铛）。
> 原来"点摘要跳 `/announcement?open=<id>` 自动展开"的能力**搬到铃铛面板**（点某条通知即带 `open` 跳转）。下表两处涉及首页摘要的验证记录同步作废。
...
| 页面（首页） | ~~公告摘要卡 → 点击跳 `/announcement?open=<id>` 并自动展开详情~~ **T19 已作废**（首页不再放公告摘要）；该能力改为铃铛面板点通知直达 |
...
#### T16 实现结果（2026-09-26 完成）

**① 交付物落地**

| 交付物 | 实现 |
|---|---|
| 提交接口（免登录） | `POST /feedback/submit`：白名单只放**两段**路径 `/feedback/*`，所以它免登录，但**必须带一次性图形验证码**（复用 `CaptchaValidator`，与 F-001/F-005 同源）。`source` 只做字典 `feedback_source` 的启用项校验 |
| 提交入口（两处） | ① **报名结果态**（公开页，未登录也能提）——结果卡下方一行「用着不顺手？说两句（不用登录）」；② **工作台**「意见反馈」卡。两处共用一个 `components/FeedbackDialog.vue`（内含验证码与刷新）；来源分别为 1 / 2 |
| 管理端列表 | `GET /feedback/admin/list` + `/admin/feedback` 页面：来源、处理状态可筛，**新→旧**；来源文案走字典（改字典立即生效）；桌面表格 / 窄屏卡片流两套形态 |
| 标记已处理 | `POST /feedback/admin/handle`（`handled` 0/1，可来回切）——PRD 只要求"可查看"，这一列是社长确认加上的（§6 D125） |
| 工作台「反馈待查看」 | 社长团 / 超管多一张卡显示**未处理条数**（复用列表接口 `handled=0&size=1` 取 total），点击进列表。T19 留的占位卡就此接上 |
| 安全边界 | 三段路径 `/feedback/admin/**` 不在白名单 → 未登录 `40100`、普通成员/部长 `40300`；内容按**纯文本**存取，不做富文本（与公告不同），前端 `{{ }}` 渲染天然转义 |

**② 实测验证记录（接口 27 项 + 页面 26 项 + 回归 106 项 = 159 项全过）**

| 分组 | 关键断言 |
|---|---|
| 接口 · 免登录提交 | 不带 token 提交 → `200`；`source=1/2` 都能提交；message = 「感谢反馈，我们会尽快查看」 |
| 接口 · 真失败仍是错误码 | 验证码错 `40001`；内容空 / 超 1000 字 `40000`；来源不在字典 `40000`；联系方式超 64 字 `40000` |
| 接口 · 权限 | 未登录看列表 `40100`；**普通成员与部长都 `40300`**；社长团 / 超管 `200`；成员标记处理 `40300` |
| 接口 · 列表与筛选 | 刚提交的在前、新→旧；来源筛选 / 未处理筛选生效；`size=9999` 被夹到 100；不存在 id 标记 → `40400` |
| 接口 · 纯文本 | 带 `<img onerror=...>` 的内容原样入库并按文本返回（前端转义即安全） |
| 页面 · 报名结果态 | 结果态有明确写「不用登录」的入口 → 点开弹窗 → 弹窗**自己取一张验证码** → 提交 → 提示「感谢反馈」+ 弹窗自动关闭 |
| 页面 · 工作台 | 「意见反馈」卡可点开提交（已登录）；「反馈待查看」数字与该账号可见的未处理条数**一致** |
| 页面 · 管理端 | 菜单里自动出现「反馈列表」（路由一处声明）；刚提交两条都在列表里；来源显示字典中文；标记前后状态标签 未处理 → 已处理、按钮反转 |
| 页面 · 窄屏 | 375 下卡片流渲染、**零横向溢出** |
| 页面 · 权限 | 普通成员菜单里**没有**「反馈列表」；直输 `/admin/feedback` 被挡回工作台 |
| 回归 | T19 42 项（菜单/卡片预期已随本轮同步更新）、T18 公开端 35 项、T15 看板 29 项，全过；`eslint` 0 error、`vite build` 通过 |
| 落库复核 | 匿名提交的 `created_by` 为 **NULL**（审计填充对无登录上下文安全）、空联系方式归一为 NULL、HTML 原样存文本 |

**③ 注意**
- ⚠️ **免登录接口的门槛就是那张验证码**：没有它，`/feedback/submit` 就是个对外的无限写接口。本轮按社长确认**不加频率限制**（§6 D123）。
- ⚠️ **弹窗用 Element Plus 而不是 Vant**：公开页（报名结果态）与工作台共用一个组件，用全站主库可避免维护两套弹窗。若觉得公开页里风格不搭，可以再做一个 Vant 版。
- ⚠️ **T19 的用例预期已同步更新**：工作台多了「反馈待查看」卡、菜单多了「反馈列表」，这两处断言在原脚本里就地改了。
- 💡 **文案前端自持**：axios 拦截器只脱壳 `body.data`，读不到后端 `message`（§6 D115），所以「感谢反馈，我们会尽快查看」在前端写了一份，与后端保持一致。
- 📌 演示数据：库里留了 2 条反馈（1 条未处理 / 1 条已处理），方便手工核对列表与筛选；自测产生的临时数据已清掉。

**验证步骤**（在你自己的栈上验：后端 8080 + 前端 5173）

```powershell
# 前提：MinIO / Redis / MySQL 起着；**本轮改了后端**，需要重启后端（IDEA 里重启即可，无新增依赖）
# 1) 免登录提交：开无痕窗口打开 http://127.0.0.1:5173/（报名开关开着会落在报名页）
#    → 用待审手机号 13900000001 提交报名 → 结果态下方点「说两句（不用登录）」→ 填内容 + 验证码 → 提交
#    → 应提示「感谢反馈，我们会尽快查看」且弹窗自动关闭
# 2) 工作台提交：用 13900000091（成员）登录 → 工作台「意见反馈」卡 → 同样能提交
# 3) 管理端：用 13900000097（社长团）或 13900000090（超管）登录 → 左侧菜单「反馈列表」
#    → 能看到刚提交的反馈（来源列显示「报名成功页 / 成员端」）；点「标记已处理」→ 状态变已处理、按钮变为「标记未处理」
#    → 来源 / 状态两个下拉筛选各试一次
# 4) 权限：用 13900000092（部长）登录 → 菜单里**没有**「反馈列表」；手输 http://127.0.0.1:5173/admin/feedback → 应提示无权并回工作台
...
#### T18 实现结果（2026-09-26 完成）

**① 交付物落地**

| 交付物 | 实现 |
|---|---|
| 三页平级互链 | 报名页 / 查询页底部加 `.apply__links` / `.query__links`（「已有账号？去登录」+ 互链另一页）；登录页底部文案统一为「去报名 / 查审核进度」。公开页不做全局导航栏 |
| 报名结果态双出口 | 结果卡按 `data.nextAction` 渲染：`QUERY` → 主按钮「去查询审核进度」+ 次按钮「已有账号？去登录」；`LOGIN` → 主按钮「去登录」+ 次按钮「去查询审核状态」 |
| 查询页按状态给主操作 | 已通过 → 「去登录」（文案写清"报名手机号 + 审核台发放的初始密码"，并把手机号预填到登录页）；已拒绝 → 「重新报名」；未找到 → 「去报名」；待审 → 只留「再查一次」 |
| 落地页按开关动态决定 | 路由表去掉写死的 `{ path: '/', redirect: '/login' }`；改由守卫判定：已登录 → `/home`，未登录按 `recruit_open` 决定 `/apply` 或 `/login`（见 §6 D110 / D114） |
| 引导分支结构化 | `POST /recruit/apply` 的「已提交过报名」「已是正式成员」由 `40000` 改为 `200` + `data.state`（`ALREADY_PENDING` / `ALREADY_MEMBER`）+ `data.nextAction`（`QUERY` / `LOGIN`）+ `data.message`（见 §6 D111 / D113） |
| 配置缓存 | 新增 `web/src/stores/recruit.js`：会话级缓存 `/recruit/info`，守卫判落地页与报名页/查询页共用一次请求；取不到开关时**不缓存失败结果**，按更保守的登录页回落（见 §6 D115） |
| 报名页简介精简 | 顶部由社长亲笔 397 字三段长文改为**一句话**「天津中德开源鸿蒙社·搞开源、作项目、共成长」（`club_intro`，21 字，后台可改）；样式改为轻量一行（左侧主色竖条 + 品牌淡底，去掉 `text-align: justify`）。后台若改回多段仍按空行分段渲染。顺带同步 **「纳新设置」页的过时提示**（原写「建议 3 段：定位 / 方向 / 纳新期待」→「建议一句话」）与文本框默认高度（rows 12→2），库中 `sys_config.remark` 一并同步（见 §6 D117） |

**② 实测验证记录（接口 35 项 + 页面 39 项 +「纳新设置」页 6 项 = 80 项全过）**

| 分组 | 关键断言 |
|---|---|
| 接口 · 四种结果 | 新手机号 → `200/SUBMITTED/QUERY`；待审 → `200/ALREADY_PENDING/QUERY`（**不再是 40000**）；已通过 → `200/ALREADY_MEMBER/LOGIN`；被拒后重提 → `200/RESUBMITTED/QUERY`（含 `message` 逐条核对） |
| 接口 · 被拒重提走真实链路 | 提交新报名 → 审核台查到该记录 → 超管拒绝 → 再提交 → `RESUBMITTED`；落库复核：`status` 回 0、`reject_reason/reviewer_id/reviewed_at` 清空 |
| 接口 · 真失败不被吞 | 验证码错 → `40001`；学院 code 非法 → `40000`；手机号格式非法 → `40000`；报名开关关闭 → `40300`（走错误码，没被引导分支吃掉） |
| 接口 · 回归 | `/recruit/status` 未找到仍 `200 + data:null`、已通过 `status=1`、已拒绝带原因；待审手机号的重复提交**没有写库**（`status` 仍 0） |
| 页面 · 落地页 | 开关开：`/` → `/apply`；开关关：`/` → `/login`；已登录：`/` → `/home` |
| 页面 · 三页互链 | 报名页 ↔ 登录页 ↔ 查询页 四处互链全部点通；三页均为公开外壳、**`.member-layout / .admin-layout` 数量为 0**（不含登录后才有的元素） |
| 页面 · 简介精简 | 报名页顶部 `.apply__intro-p` **恰好 1 条**、文案等于一句话介绍（21 字、无换行）；原 397 字长文不再出现 |
| 页面 ·「纳新设置」页同步 | 社团简介字段提示已改为「建议一句话」（不含"建议 3 段"）、文本框默认 `rows=2`、其中正是该句、库中 `sys_config.remark` 同步；**6 项全过** |
| 页面 · 结果态 | 用真实手机号 13900000001（待审）在 `dist` 产物上真实填表提交：标题「你已提交过报名」、按钮 `['去查询审核进度','已有账号？去登录']`、**`.el-message--error` 数量 0**（旧行为会弹红色报错）、文案含后端下发内容与手机号回显、点次按钮 → `/login` |
| 页面 · 查询页三态 | 已通过 → 标题 + 「去登录」+ 文案含「初始密码」+ 跳登录页且**手机号已预填**；已拒绝 → 「重新报名」；未找到 → 「去报名」 |
| 工程 | `npm run lint` 0 error、`npm run build` 通过 |

**③ 注意**
- ⚠️ **本轮踩到一个测试工具坑（值得复用）**：自写的极简 RESP 客户端**在第一个数据包就 resolve**，而 `KEYS` 是多值回复 →
  读成空数组，误判成"页面没取验证码"（Redis 里其实有 8 个历史 key）。改为「等回复空闲 250ms 再收」后正常。
  **教训：凡多值/可能分包回复的协议，别按第一个 chunk 收结果**（`lib.cjs` 的 `redisGet` 当时只读单值、没暴露这个问题）。
- ⚠️ **`npm run build` 可能失败在清空 `dist` 上**（不是代码问题）：沙箱的 safe-delete 垫片会 `spawnSync ... ETIMEDOUT` →
  报 `[plugin vite:prepare-out-dir]`。**先 `rm -rf web/dist` 再 build** 即可绕开（构建产物目录，`dist/` 已被 `.gitignore` 忽略）。
- 报名页「已提交过报名」这条引导来自**后端 state**，不是前端猜的 —— 前端只决定标题/图标/按钮文案，`nextAction` 决定去哪。
- 自测产生的两条临时数据（`13900000098` / `13900000099`）已删除；库中 `recruit_apply` 9 条、`user` 9 人、`recruit_open=1` 与自测前一致。
- ⚠️ **又一个测试工具坑（值得复用）**：无头浏览器的登录态存在 **Chrome 的 `user-data-dir`** 里 —— 跑过一次「已登录」用例后，
  同一 profile 再跑公开页用例会**整体带跑偏**（`/` 直接进 `/home`，14 项假失败）。**每个模式/用例要用独立 profile**（已按 `--open`/`--closed`/`--logged` 分开）。
- 📌 **原 397 字长简介的去向**：已从配置与种子里移除，需要时用 git 回溯（`git show 90ddafe:server/sql/02_seed_dict.sql`）。
  它属于 PRD §10.1 的「报名页内容素材」，将来做官网/落地页可以直接复用；社长若希望**留在仓库里**，说一声我另找位置放。

**验证步骤**（在你自己的栈上验：后端 8080 + 前端 5173）

```powershell
# 前提：MySQL / Redis 常驻；MinIO 起着；后端在 IDEA 里重启一次（本轮改了 server 三个文件）
#       前端 npm run dev（5173 会自动热更新）
$mysql = "C:\Program Files\MySQL\MySQL Server 8.0\bin\mysql.exe"

# 1) 报名开关打开时：浏览器手机模拟打开 http://127.0.0.1:5173/（注意是根路径，不是 /login）
#    → 应直接落在「入社报名」页（而不是登录页）
# 1b) 报名页顶部应只有「一句话介绍」一行（天津中德开源鸿蒙社·搞开源、作项目、共成长），紧接着就是表单
#     —— 想改这句文案：超管 →「纳新设置」页 →「社团简介」；改回多段也能正常按空行分段渲染
# 2) 报名页底部点「已有账号？去登录」→ 登录页；登录页底部「去报名 / 查审核进度」两个入口都能点通
# 3) 在报名页用「已提交过报名」的手机号（如库里待审的 13900000001）填完提交
#    → 出现「你已提交过报名」结果卡（橙色 i 图标）+「去查询审核进度」/「已有账号？去登录」两个按钮，
#      且**不弹红色报错**（这是本轮最关键的改动）
# 4) 查询页用 13900000002（已通过）查 → 主按钮「去登录」，点进去登录页手机号已预填
#    用 13900000003（已拒绝）查 → 「重新报名」；用未报名手机号查 → 「去报名」
# 5) 关掉报名开关再开根路径（只改库，验证完记得改回 1）：
& $mysql --user=root --password=root --default-character-set=utf8mb4 "--execute=USE osc; UPDATE sys_config SET config_value='0' WHERE config_key='recruit_open';"
#    → 刷新 http://127.0.0.1:5173/ 应落在登录页；验完改回：
& $mysql --user=root --password=root --default-character-set=utf8mb4 "--execute=USE osc; UPDATE sys_config SET config_value='1' WHERE config_key='recruit_open';"
```

> 说明：也可以用超管在「纳新设置」页开关报名，效果一样（`/config/admin/update`）。


### T19 内部导航与首页工作台 ✅

> **⚠️ 范围变更（2026-09-26 社长拍板）**：原第 6 条「**不做**：统一外壳」（记在 D112）**当场被推翻** ——
> 社长明确「就不分管理端和成员端了吧？都是根据账号权限显示能看到的和能设置的东西，核心在权限而不是页面了」。
> 于是本任务点**把统一外壳一起做掉**（一次做完反而省：否则要在两套外壳里各做一遍菜单生成，合并时还得再动一次 layout）。
> 两个取舍由社长选定：**保留现有路径**（`/admin` 退化为纯命名空间）+ **移动端底部 3 项 +「更多」抽屉**。

- **覆盖**：PRD §8.2 v3.2、§6 D106~D108 / D112 / D118~D122
...
- **原第 6 条「不做：统一外壳」已作废并落地**：三套 layout 合并为 `PublicLayout`（公开场景）+ `AppShell`（内部场景）
  —— 详见下方「实现结果」与 §6 D118。**统一外壳不再是待办。**
...
#### T19 实现结果（2026-09-26 完成）

**① 交付物落地**

| 交付物 | 实现 |
|---|---|
| 统一外壳 | **删除 `MemberLayout.vue` / `AdminLayout.vue`**，新增 `layouts/AppShell.vue`：桌面左侧菜单、移动端底部 tabbar（高频 3 项）＋「更多」抽屉；全部内部页面共用它。「进入管理端」「返回成员端」两个按钮**消失** |
| 菜单单一出处 | 路由 meta 声明 `{ title, task, menu: { label, order, capability } }`；新增 `router/menu.js` 的 `useRouteMenu()`：用 `router.getRoutes()` 过滤 `capability` + 按 `order` 排序生成菜单。**加一个页面只改路由表一处** |
| 能力判定收敛 | `constants/roles.js` 按 PRD 权限矩阵逐行补具名能力函数（12 个）；**store 上旧的 4 个计算属性（`isSuperAdminUser` / `canManageAllUsers` / `canManageDeptUsers` / `canEnterAdminPage`）全部移除**；守卫 3 段 if 合并为一条 `meta.menu.capability` 判定；`AuditView.isFullScope` 改为 `canManageAll(profile)` |
| 首页改工作台 | 撤掉公告摘要卡；改为欢迎卡 + 按角色卡片（待审报名 / 资料完整度 / Excel 导入 / 数据导出 / 意见反馈占位） |
| 公告通知红点 | 新增 `components/AnnouncementBell.vue`（右上角铃铛 + 红点 + 面板内「一键已读」）；`utils/announcementRead.js` 统一读写已读时间；`AnnouncementView` 挂载即写已读 |
| 路径与移动端 | 保留现有路径（`/admin` 退化为命名空间）；移动端 = 底部按 `order` 取前 3 项 + 「更多」 |

**② 实测验证记录（T19 页面 42 项 +「只改一处」端到端 4 项 + 回归 98 项 = 144 项全过）**

| 分组 | 关键断言 |
|---|---|
| 四角色菜单 | 成员 / 部长 / 社长团 / 超管 的左侧菜单与预期**逐项逐序完全一致**（差异项：`/admin/dict` 只超管、`/admin/import` 只社长团+超管、`/admin/audit` 部长起） |
| 菜单 ⇔ 守卫一致 | 菜单里**每一项都真的点得进**；无资格的路由直接输 URL **被挡回工作台**（四角色各测一组） |
| 统一外壳 | 四角色均为 `.app-shell`、`.member-layout / .admin-layout` 计数为 0、页面不再出现「进入管理端 / 返回成员端」；从 `/admin/audit` 点菜单「个人中心」→ `/profile` 且**外壳不换** |
| 工作台 | 四角色卡片集合与预期一致（成员：资料完整度 + 反馈占位；部长：+ 待审报名；社长团/超管：+ 导入 / 导出） |
| 移动端 | 375 宽下 tabbar = 「工作台 / 审核管理台 / 公告 / 更多」；「更多」抽屉里是全部菜单 |
| 红点 | 清掉已读时间 → 红点出现；打开公告页 → 红点消失且**刷新不回来**；面板有「一键已读」，点了红点消失 |
| 「只改一处」端到端 | **临时往路由表加一条路由 → 重建 → 菜单自动多出该页且能点进去**（未改任何菜单代码）；验完即删，构建产物里 `grep tmp-menu-check` = 0 |
| 回归 | T18 公开端 25 + 10、T15 看板 29、T13 导入 34（内含 T12 公告管理页 / T10 成员档案页可打开）全部通过 |

**③ 注意**
- ⚠️ **口径变更：看板对成员开放了**（取代 D103）。PRD 权限矩阵「看板查看」本来就给成员 ✅；D103 当初写"成员端导航不加项"是**两套外壳时代的折中**，统一外壳后按权限显示，成员菜单里就有「看板」。若要收回，只改路由表该条的 `capability` 一行。
- ⚠️ **T15 的旧断言已就地更新**：它把 D103 写死成「普通成员被管理端守卫拦住」，本轮改为「成员能进看板页」。
- ⚠️ **T12 的记录已订正**：撤掉首页公告摘要卡后，原来"成员端首页有公告摘要"的描述失效 → 已在该条目注明（公告入口改为「公告」菜单 + 右上角铃铛）。
- ⚠️ **已读时间的格式必须对齐后端 `LocalDateTime`**（本地时区、无 `Z`、秒级）。用 `toISOString()`（UTC）会比后端早 8 小时，导致"刚看完又亮红点"；已封装在 `utils/announcementRead.js#nowLocalIso()`。
- ⚠️ **历史页面用例脚本里的外壳选择器全部失效**（`.admin-aside / .admin-menu / .admin-actions / .member-tabbar` → `.app-aside / .app-menu / .app-actions / .app-tabbar`）。这些脚本在 `%TEMP%` 下、不入库；本轮已就地改掉 T13 / T15 的。
- 🐛 **两个测试工具教训（已进交接文档 §四）**：① Chrome 的 `user-data-dir` 残留会让整套用例跑偏（`/` 直接进 `/home`，一次假失败 14 项），甚至**静默退出**（CDP 连不上就直接结束、不报错）→ 每个用例用独立 profile 或先清 profile；② **T13 的上传用例不自清理**（跑完留下 2 个测试账号 `13910000301/302`），重复跑会让断言失败 → 收尾必须手工核库。

**验证步骤**（在你自己的栈上验：后端 8080 + 前端 5173）

```powershell
# 前提：MinIO / 后端 / 前端起着（本轮**没有任何后端改动**，不用重新打包）
# 1) 四种账号分别登录，看左侧菜单是不是"恰好"自己能看的那些：
#    13900000090 超管 11 项 / 13900000097 社长团 9 项 / 13900000092 部长 8 项 / 13900000091 成员 5 项
#    （密码见每日日志 OscTest#2026）
# 2) 菜单里每一项都点一遍 —— 不该出现"点不进去"；再手输一个没资格的路由
#    （例如成员输 http://127.0.0.1:5173/admin/dict）→ 应提示「没有访问「字典管理」的权限」并回到工作台
# 3) 工作台：换账号对比卡片（成员没有「待审报名」；社长团/超管有「Excel 导入 / 数据导出」；「意见反馈」是 T16 占位）
# 4) 右上角铃铛：先在浏览器控制台执行下面这行再看工作台，应出现红点
#    localStorage.removeItem('osc_announcement_read_at')
#    → 打开「公告」页后红点消失、刷新也不回来；铃铛面板里的「一键已读」同样能消红点
# 5) 手机模拟（窗口宽度 ≤768）：底部应是 3 个高频项 +「更多」；点「更多」抽屉里是全部菜单
```
...
| D121 | 公告红点口径：**有新公告就亮，看公告页才算已读** | ① 红点判定 = 最新公告的发布时间 > `localStorage` 里记的上次查看时间（**不设 N 天窗口**，社长拍板"只要有新通知就亮"）。② 已读时机 = **打开「公告」页**（`AnnouncementView` 挂载写时间）；铃铛面板另给「**一键已读**」按钮。③ 铃铛面板还能直接点某条公告跳到 `/announcement?open=<id>` 展开详情（原首页摘要卡的能力搬到这儿）。④ ⚠️ **时间格式必须与后端 `LocalDateTime` 一致**（本地时区、无 `Z`、秒级）—— 用 `toISOString()` 会差 8 小时，导致"刚看完又亮红点"；已封装 `utils/announcementRead.js#nowLocalIso()` | T19 |
| D122 | 工作台按角色的待办与入口（反馈入口先占位） | 首页不再承载内容（D107），改为**按角色的卡片**：**全员** = 资料完整度（列出还缺学号 / 生源地 / 个人简介，点了去个人中心）+ 意见反馈；**部长起** ＋待审报名数（数字来自 `/recruit/admin/stats`，**按可见范围**：部长=本部门、社长团/超管=全社）；**社长团/超管** ＋Excel 导入、数据导出入口。⚠️ 两个实现口径：① 成员打开工作台**不会**去请求 `/recruit/admin/stats`（否则拿到 40300），靠 `canReviewRecruit(profile)` 前置判断；② 反馈入口**只放明确标注"T16 待接入"的占位卡**，不假装有数字（社长拍板"占位吧，后续也好定位"） | T19 / T16 |
| D123 | 免登录提交的**门槛 = 一次性图形验证码**，本轮不加频率限制 | F-014 要求"提交无需登录"，但 `/feedback/submit` 一旦免登录就是个对外的写接口 —— 所以第一道门复用 F-001/F-005 那套 `CaptchaValidator`（每次提交消耗一张，等于把成本抬到"得先算对一道算术题"）。**是否再加"同 IP 每分钟 N 条"**：社长确认**本轮不加**（验证码已够，V1.0 不引入额外限流组件）。安全边界仍完整：三段路径 `/feedback/admin/**` 不在白名单，未登录 `40100`、普通成员/部长 `40300` | T16 |
...
> - 2026-09-26 **T15 提交推送**（`1687fa7` server / `82350fa` web / `90ddafe` docs），工作树回到干净状态。
> - 2026-09-26 **T18 完成**（公开端报名链路梳理：三页平级互链 + 报名结果态双出口 + 查询页按状态给主操作 + 根路径按 `recruit_open` 动态落地 + `POST /recruit/apply` 的两个引导分支改 `200 + nextAction`；**附带**：报名页简介由 397 字长文精简为**一句话**，并同步「纳新设置」页的过时提示）。接口 **35 项** + 页面 **39 项** +「纳新设置」页 **6 项**实测全过（页面用例在 `dist` 产物上真实填表提交）；`eslint` 0 error、`vite build` 通过。T18 条目补「实现结果 + 验证记录」，总表状态转 ✅；顺带按 D111 的代价**同步订正 T6 的实现结果三处**（另加简介行）；§6 新增 D113~D117。
> - 2026-09-26 **T18 提交推送**（`bb842e4` server / `0806428` web 公开端 / `2ede23d` 简介精简 / `0ce15b7` docs），工作树回到干净状态。
